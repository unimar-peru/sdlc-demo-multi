# Guía de Contribución — sdlc-demo-multi
...
3. **Frontera de carpetas.** Documentación en `reference/`, planificación en `docs/`, código ejecutable en `src/`. No mezclar.
...
6. **Build y pruebas del runtime.**
   ...
   ```bash
   # Este satélite todavía no aloja código ejecutable (G-008). Hasta que
   # `src/services/` tenga su primer servicio, la «build» es el lint documental
   # y las «pruebas» son el barrido de ubicación y del modelo Suite → Sistema.
   ...
   Al publicarse el primer servicio, estos dos bloques se sustituyen por la build
   y las pruebas reales del runtime, y los mismos comandos se encadenan al final
   de [`.husky/pre-commit`](.husky/pre-commit) y del workflow de gobernanza.
...
3. **Abre la propuesta como PR en `unimar_arch`**, contra la fuente del estándar (`.harness/`), nunca contra una copia local. Si el cambio es una decisión técnica, lleva su ADR.
   ...
   ```bash
   gh pr create --repo unimar-peru/unimar_arch --title "..." --body "Origen: sdlc-demo-multi, gap G-NNN"
   ```
...
| :--- | :--- | :--- |
| `src/` | Código ejecutable del satélite | Manda la convención del runtime, no `kebab-case` |
...
| :--- | :--- | :--- | :--- |
| 1.0.0 | 2026-08-06 | sdlc-demo-multi | Materialización desde la plantilla del plugin `unimar-core` |
